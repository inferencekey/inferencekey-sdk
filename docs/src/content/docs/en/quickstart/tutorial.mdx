...
InferenceKey separates **control** from **data**. You provision workloads with a control token (`ik_sdk_`) and call inference with a data token (`ik_live_`). You will use both below. See [Tokens & scopes](/en/quickstart/tokens/) for the full model.
</Aside>
...
2. ### Get your tokens
   ...
   Using the wrong token type returns a `403`: `wrong_credential_type` (data token on a control route or vice-versa), `project_scope_mismatch`, or `scope_insufficient`. See [Common errors](/en/reference/common-errors/) if you hit one.
   </Aside>
...
4. ### Provision a workload with `ensure()`
   ...
   `on_drift` defaults to `OnDrift.RECONCILE` — if the live workload no longer matches your spec, `ensure()` brings it back into line. Other modes (`FAIL`, `DRY_RUN`, `WARN`, `IGNORE`) let you observe instead of change. See [OnDrift](/en/reference/ondrift/).
   </Aside>
...
<Aside type="tip" title="Delete it when you're done">
Closing the loop: `mgmt.delete("support-bot")` removes the workload — and, for an `autoscaling`/`scheduled` one, tears down the cloud GPU the platform provisioned, so you stop paying for it. It's idempotent, so put it in a `finally` (or a shutdown handler) to clean up on exit. See [`delete`](/en/api/python/#delete) ([TypeScript](/en/api/typescript/#mgmtdeleteworkloadslug-opts)).
</Aside>

## Next steps

<CardGrid>
  <LinkCard title="Tokens & scopes" href="/en/quickstart/tokens/" description="The full least-privilege model: ik_sdk_ vs ik_live_ and how scopes resolve." />
  <LinkCard title="Your first ensure()" href="/en/quickstart/first-ensure/" description="Go deeper on declarative provisioning and reconciliation." />
  <LinkCard title="Your first call" href="/en/quickstart/first-call/" description="More on the data plane, including embeddings with embed()." />
  <LinkCard title="OnDrift modes" href="/en/reference/ondrift/" description="RECONCILE, FAIL, DRY_RUN, WARN, IGNORE — pick the safety level you want." />
  <LinkCard title="Backends & policies" href="/en/reference/backends-and-policies/" description="vLLM, Ollama, SGLang, plus fixed / scheduled / autoscaling execution policies." />
  <LinkCard title="Use cases" href="/en/guides/use-cases/" description="Patterns for chat, embeddings, and the other task types." />
</CardGrid>

<Aside type="tip" title="Embeddings, too">
The same `DataClient` does embeddings. For example, in Python: `data.endpoint("billing", api_key="ik_live_...").embed(input=["a", "b"]).embeddings`. See [Your first call](/en/quickstart/first-call/).
</Aside>
