...
<CardGrid>
  <Card title="ik_sdk_ — Control plane" icon="setting">
    Provisions and reconciles workloads. Held by your **management client** and
...
Both live in the [Manager dashboard](https://manager.inferencekey.com), but on different screens because they have different blast radius.

<Steps>

1. **`ik_sdk_` — the Tokens screen.**
   Open your project in the Manager and go to **Tokens**. Create an SDK token; it is bound to that project. This is the value your `ManagementClient` reads.

2. **`ik_live_` — the API Keys screen.**
   Go to **API Keys** and mint a key for the workload(s) you want an app to call. Hand each app the narrowest key it needs. This is the value you pass to `data.endpoint(...)`.
...
3. **Configuration file** — falls back to your local config when neither of the above is set.
...
New to InferenceKey? [Create an account or open the dashboard](https://manager.inferencekey.com) to mint your tokens. Learn more at [inferencekey.com](https://inferencekey.com).
