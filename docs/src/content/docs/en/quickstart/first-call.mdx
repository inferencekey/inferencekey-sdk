...
Your workload exists on the platform (see [Your first ensure](/en/quickstart/first-ensure/)). Now call it.
...
<Aside type="caution" title="Two tokens, two clients">
  `ik_sdk_` (control) provisions workloads and **cannot** call inference. `ik_live_` (data) calls inference and **cannot** provision. The `DataClient` only ever takes an `ik_live_` key. See [Tokens](/en/quickstart/tokens/).
</Aside>
...
<Aside type="note" title="Match the method to the workload">
  `generate_text` targets text-generating workloads; `embed` targets embedding workloads. Each workload declares a `task_type` at `ensure()` time, so reach for the method that matches. See [Workloads by policy, worker & modality](/en/guides/workloads-by-policy-worker-modality/).
</Aside>
...
Under the hood the endpoint speaks **server-sent events**, terminated by `data: [DONE]`; the SDK parses those frames into `TextChunk`s for you. For the raw wire contract see the references below.

<CardGrid>
  <LinkCard
    title="Wire format"
    href="/en/reference/wire-format/"
    description="The OpenAI-compatible data-plane routes, SSE framing, and the [DONE] terminator."
  />
  <LinkCard
    title="Use cases"
    href="/en/guides/use-cases/"
...
The SDK raises typed errors — `AuthError`, `PermissionDenied`, `ValidationError`, `ApiError` — all subclasses of `InferenceKeyError`. See [Common errors](/en/reference/common-errors/).
