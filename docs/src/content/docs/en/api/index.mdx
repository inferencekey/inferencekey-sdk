...
<Aside type="tip" title="Configuration precedence">
  Every client resolves settings in the order **explicit argument → environment variable**. The relevant env vars are `INFERENCEKEY_BASE_URL`, `INFERENCEKEY_PROJECT`, `INFERENCEKEY_API_KEY` (default `ik_live_`), and `INFERENCEKEY_SDK_TOKEN`. See [Authentication](/guides/authentication/).
</Aside>
