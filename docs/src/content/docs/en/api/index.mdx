...
See [Tokens](/reference/tokens/) for the least-privilege model and [Architecture](/reference/architecture/) for how the core, ABI, and bindings fit together.
...
- **`Backend`** — `Ollama`, `Vllm`, `VllmOmni`, `Sglang`. See [Backends and policies](/reference/backends-and-policies/).
- **`OnDrift`** — `Reconcile` (default), `Fail`, `DryRun`, `Warn`, `Ignore`. See [OnDrift](/reference/ondrift/).
- **Errors** — `PermissionDenied`, `AuthError`, `ValidationError`, `ConfigurationError`, `ApiError`, all subclasses of `InferenceKeyError`. See [Common errors](/reference/common-errors/).

<Aside type="tip" title="Configuration precedence">
  Every client resolves settings in the order **explicit argument → environment variable → config file**. The relevant env vars are `INFERENCEKEY_BASE_URL`, `INFERENCEKEY_PROJECT`, `INFERENCEKEY_API_KEY` (default `ik_live_`), and `INFERENCEKEY_SDK_TOKEN`. See [Authentication](/guides/authentication/).
</Aside>
...
1. Get your tokens and learn the two-token model — [Tokens quickstart](/quickstart/tokens/).

2. Declare your first workload with `ensure()` — [First ensure](/quickstart/first-ensure/).

3. Call the endpoint it created — [First call](/quickstart/first-call/).

</Steps>

You'll need a project and an `ik_sdk_` token to follow along. <a href="https://manager.inferencekey.com">Create an account or open the dashboard</a> to provision them.

---

Looking for the bigger picture? Visit the [InferenceKey landing site](https://inferencekey.com).