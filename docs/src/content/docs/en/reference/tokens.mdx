...
<CardGrid>
  <Card title="ik_sdk_ — control plane" icon="setting">
    Provisions and reconciles workloads. Held by your **ManagementClient**,
...
1. **Create in the dashboard.** Both `ik_sdk_` and `ik_live_` tokens are minted in
   the [dashboard](https://cloud.inferencekey.com). Control tokens are
...
2. **Copy it once.** The full secret is **shown only at creation time**. The
   dashboard stores a hash, not the raw token — if you lose it, you cannot read it
   back, you rotate it.
