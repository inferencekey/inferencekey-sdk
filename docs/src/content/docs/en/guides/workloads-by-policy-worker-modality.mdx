...
Examples assume `INFERENCEKEY_SDK_TOKEN` (control plane) and `INFERENCEKEY_PROJECT=acme` are exported. The `ik_live_` data keys are passed per workload. See [Tokens](/en/quickstart/tokens/) and [Authentication](/en/guides/authentication/).
</Aside>
...
| --- | --- | --- | --- |
| Chat / completion | `text2text` | `vllm`, `sglang`, `ollama`, `llamacpp` | `generate_text()` |
...
The default modality. `vllm` is the workhorse for served HF models; `sglang` for high-throughput serving; `ollama` and `llamacpp` for GGUF models (`llamacpp` runs `llama-server` directly, strong on AMD/ROCm and Apple Silicon).
...
<Steps>
1. **Modality** picks `task_type` + `backend` + `command`. Text generation (`text2text`) rides on `vllm`/`sglang`/`ollama`/`llamacpp`; embeddings on `vllm`/`ollama`; image/audio (`text2image`, `text2audio`, `audio2text`) ride on `vllm-omni`.
...
4. `ensure()` is idempotent by `slug`, so re-running with a changed spec reconciles in place (default `OnDrift.RECONCILE`). See [OnDrift](/en/reference/ondrift/).
</Steps>

<CardGrid>
  <LinkCard title="Backends & policies" href="/en/reference/backends-and-policies/" description="Every backend wire string and the policy contract in detail." />
  <LinkCard title="Wire format" href="/en/reference/wire-format/" description="The exact control and data-plane routes behind ensure() and the endpoints." />
  <LinkCard title="OnDrift" href="/en/reference/ondrift/" description="How re-running ensure() reconciles a changed spec." />
  <LinkCard title="Common errors" href="/en/reference/common-errors/" description="403 wrong_credential_type, scope_insufficient, and friends." />
</CardGrid>
