...
<Aside type="note">
The shape of `execution_policy_config` is validated by the Manager and may evolve; the keys shown below (`replicas`, `cron`/`window`, `min_replicas`/`max_replicas`) are illustrative. The policy *name* is the contract. See [Backends & policies](/reference/backends-and-policies/).
</Aside>
...
# Autoscaling (cloud): replica count tracks load.
mgmt.ensure(WorkloadSpec(**base,
    execution_policy=ExecutionPolicy.AUTOSCALING,
    execution_policy_config={"min_replicas": 0, "max_replicas": 5}))
...
await mgmt.ensure({ ...base,
  ...
  executionPolicyConfig: { min_replicas: 0, max_replicas: 5 } });
...
# Autoscaling (private worker): scale on your own hardware.
mgmt.ensure(WorkloadSpec(**base, worker_id="wrk_7Yc2...",
    execution_policy=ExecutionPolicy.AUTOSCALING,
    execution_policy_config={"min_replicas": 1, "max_replicas": 4}))
```
...
await mgmt.ensure({ ...base, workerId: "wrk_7Yc2...",
  executionPolicy: "autoscaling",
  executionPolicyConfig: { min_replicas: 1, max_replicas: 4 } });
```
...
ref = mgmt.ensure(WorkloadSpec(
    ...
    execution_policy=ExecutionPolicy.AUTOSCALING,
    execution_policy_config={"min_replicas": 0, "max_replicas": 3},
))
```
...
const ref = await mgmt.ensure({
  ...
  executionPolicy: "autoscaling",
  executionPolicyConfig: { min_replicas: 0, max_replicas: 3 },
});
```
...
mgmt.ensure(WorkloadSpec(**base,
    execution_policy=ExecutionPolicy.AUTOSCALING,
    execution_policy_config={"min_replicas": 0, "max_replicas": 4}))
...
await mgmt.ensure({ ...base,
  executionPolicy: "autoscaling",
  executionPolicyConfig: { min_replicas: 0, max_replicas: 4 } });
...
ref = mgmt.ensure(WorkloadSpec(
    ...
    execution_policy=ExecutionPolicy.AUTOSCALING,
    execution_policy_config={"min_replicas": 0, "max_replicas": 6},
))
```
...
const ref = await mgmt.ensure({
  ...
  command: "vllm-omni serve openai/whisper-large-v3",
  executionPolicy: "autoscaling",
  executionPolicyConfig: { min_replicas: 0, max_replicas: 6 },
});
```
...
<Aside type="tip">
Don't have a worker or project yet? [Create an account and open the dashboard](https://app.inferencekey.com) to register a private worker, mint `ik_sdk_` / `ik_live_` tokens, and watch your workloads come up.
</Aside>
