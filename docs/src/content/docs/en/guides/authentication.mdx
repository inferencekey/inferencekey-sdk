...
<Aside type="tip">
Mint `ik_sdk_` and `ik_live_` tokens in the **[Manager dashboard](https://manager.inferencekey.com)**. Create an account or open the dashboard there to generate keys for your project and workloads.
</Aside>
...
Both clients resolve their token and project from three sources, in this order. **Explicit argument wins, then environment variable, then a config file.**
...
3. **Config file** — on-disk defaults, used only when neither of the above is set.
...
Need keys? Open the **[Manager dashboard](https://manager.inferencekey.com)** to create an account and mint `ik_sdk_` and `ik_live_` tokens for your project. Learn more at [inferencekey.com](https://inferencekey.com).