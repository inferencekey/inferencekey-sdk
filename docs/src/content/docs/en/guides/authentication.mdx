...
If you just want to get a key and make a call, start with [Tokens](/en/quickstart/tokens/). For the API surface, see the [Python](/en/api/python/) and [TypeScript](/en/api/typescript/) references.
...
<Aside type="note">
Scopes (capabilities like `workload:write`) are a property of the **`ik_sdk_` control token only**. An `ik_live_` data token is instead gated by **which endpoints it may call**, enforced by the data-plane route `/endpoint/:projectSlug/:workloadSlug/v1/...`. Its reach is whichever endpoints it was granted: one or more specific `projectSlug/workloadSlug` (per workload — the common, least-privilege case), or a `projectSlug/*` wildcard that grants **every workload in that project, including ones created later**. The project-wide form is convenient (one key, no per-workload reassignment) but has a wider blast radius — prefer per-workload keys unless a caller genuinely needs the whole project.
</Aside>
...
<CardGrid>
  <LinkCard title="Tokens (quickstart)" href="/en/quickstart/tokens/" description="Get a key and make your first call." />
  <LinkCard title="Tokens (reference)" href="/en/reference/tokens/" description="The full token model and wire details." />
  <LinkCard title="Common errors" href="/en/reference/common-errors/" description="Every typed error and how to recover." />
  <LinkCard title="OnDrift" href="/en/reference/ondrift/" description="What ensure() does when reality diverges from your spec." />
</CardGrid>
