...
3. **Add a webhook destination.** The URL must be `https://` and publicly resolvable — see [Webhook URL requirements](#webhook-url-requirements). Its signing secret is generated at the same time and shown once; copy it before leaving the panel. Then press **Test** on the saved row to send a signed ping.
