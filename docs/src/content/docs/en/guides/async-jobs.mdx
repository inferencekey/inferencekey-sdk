...
3. **Set your webhook URL and generate a signing secret.** The URL must be `https://` and publicly resolvable — see [Webhook URL requirements](#webhook-url-requirements).
...
After the sixth failure the delivery is marked **terminal** and no longer retried. You can see terminal deliveries in the dashboard.
