...
There is one case where a ping arrives **unsigned**: testing a destination you haven't added yet, since it has no secret to sign with. If your handler rejects every unsigned request — the correct default — that first test returns 401 and the dashboard won't unlock **Add**. Two ways out, both fine:

- Add the destination first, generate its secret, then press **Test**. The ping is signed and verification passes.
- Or let your handler accept an unsigned request **only** when `X-InferenceKey-Event: ping`, and keep requiring a signature on `job`. This is what Stripe and GitHub do for their verification pings.

<Aside type="caution" title="Never accept an unsigned `job`">
Whatever you do with pings, a callback carrying `X-InferenceKey-Event: job` without a valid signature must be rejected — that's the one that carries real results.
</Aside>
