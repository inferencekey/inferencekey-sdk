"""Forecast a time series with a **TimesFM 2.5** custom backend on a private GPU.
...
(``timesfm``, the one this folder publishes) and the task is **forecast**. The
...
TimesFM is **univariate**: it forecasts from the series history alone and ignores
covariates. For a covariate-aware forecast (a stock plus VIX/volume/calendar),
see the sibling ``chronos2-forecast-private-amd-gfx120x`` example.
...
custom backend** (``"timesfm"``), not one of the built-in ``Backend`` enums, and
...
# --- The model / backend ---------------------------------------------------
# google/timesfm-2.5-200m-pytorch: a ~200M-parameter zero-shot time-series
# foundation model. It fits with room to spare on the R9700's 32 GB; the real
# constraint is host RAM at load — see README.
MODEL = "google/timesfm-2.5-200m-pytorch"

# The custom backend's published name (backend.py sets `name = "timesfm"`).
# For a custom backend this is the `backend` field — NOT a Backend enum.
BACKEND_NAME = "timesfm"

SLUG = "timesfm-forecast"
...
def main() -> int:
    ...
    try:
        ...
        #    timesfm), download the weights, and load + compile the model. The
