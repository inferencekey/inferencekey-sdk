"""Forecast a time series with a **Moirai 2.0** custom backend on a private GPU.
...
(``moirai2``, the one this folder publishes) and the task is **forecast**. The
...
This example forecasts a **univariate** series (history only, no covariates). For
a covariate-aware forecast (a stock plus VIX/volume/calendar), see the sibling
``chronos2-forecast-private-amd-gfx120x`` example.
...
custom backend** (``"moirai2"``), not one of the built-in ``Backend`` enums, and
...
# --- The model / backend ---------------------------------------------------
# Salesforce/moirai-2.0-R-small: a compact decoder-only universal forecasting
# model. It fits with room to spare on the R9700's 32 GB; the real constraint is
# host RAM at load — see README.
MODEL = "Salesforce/moirai-2.0-R-small"

# The custom backend's published name (backend.py sets `name = "moirai2"`).
# For a custom backend this is the `backend` field — NOT a Backend enum.
BACKEND_NAME = "moirai2"

SLUG = "moirai2-forecast"
...
def main() -> int:
    ...
    try:
        ...
        #    uni2ts), download the weights, and load the model. The first boot is
        #    slow, so give it a generous timeout.
